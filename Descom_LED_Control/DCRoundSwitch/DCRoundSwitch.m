...
//	CGFloat width = [onString sizeWithFont:toggleLayer.labelFont].width;
    ...
    [[NSAttributedString alloc]
     initWithString:onString
     attributes:@
     {
     NSFontAttributeName: toggleLayer.labelFont
     }];
...
//	CGFloat offWidth = [offString sizeWithFont:toggleLayer.labelFont].width;
    CGFloat offWidth = 0.0f;
    NSAttributedString *attributedOff =
    [[NSAttributedString alloc]
     initWithString:offString
     attributes:@
     {
     NSFontAttributeName: toggleLayer.labelFont
     }];
    
    CGRect offrect = [attributedOff boundingRectWithSize:(CGSize){offWidth, CGFLOAT_MAX}
                                               options:NSStringDrawingUsesLineFragmentOrigin
                                               context:nil];
    offWidth  = ceilf(offrect.size.width);
