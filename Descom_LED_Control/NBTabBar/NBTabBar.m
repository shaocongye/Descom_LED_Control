...
- (id)initWithItems:(NSArray *)items {
    self = [super init];
    if (self) {
        self.frame = CGRectMake(0, SCREEN_HEIGHT - HEIGHT_MENU_VIEW - 65, SCREEN_WIDTH, 45);
        
        self.backgroundColor = [UIColor colorWithRed:50.0f/255.0f green:85.0f/255.0f blue:180.0f/255.0f alpha:1.0f];
...
- (void)setupButtons {
    ...
    for (NBTabButton *info in self.buttonData) {
        NSInteger extra = 0;
        ...
        tabButton.frame  = CGRectMake(buttonX, 0, buttonSize + extra, 43);
...
//        UIImage *hightabButtonBackground = [[UIImage imageNamed:@"hightabButtonBackground"]stretchableImageWithLeftCapWidth:1 topCapHeight:0];
        ...
        btnImgView.frame = CGRectMake(0, 0, buttonSize + extra, 43);
...
- (void)touchDownForButton:(UIButton *)button{
    ...
    NBTabController *vc = [[self.buttonData objectAtIndex:i] viewController];
